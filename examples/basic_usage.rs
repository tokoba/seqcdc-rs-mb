...
use seq_chunking::{
    ChunkingConfig, SeqChunking, SeqOpMode,
    utils::{FileUtils, PerfUtils, TestDataGenerator, ValidationUtils},
};
...
fn example_file_processing() -> seq_chunking::Result<()> {
    ...
    println!(
        "Files identical: {}",
        if files_identical { "✓ YES" } else { "✗ NO" }
    );
...
fn example_performance_measurement() -> seq_chunking::Result<()> {
    ...
    let (chunks, duration) = PerfUtils::measure_time(|| chunker.chunk_all_vec(&data));
